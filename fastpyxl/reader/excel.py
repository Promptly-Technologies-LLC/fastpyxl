...
from fastpyxl.cell import MergedCell
...
def _validate_archive(filename):
    ...
    if not is_file_like:
        ...
        if file_format not in SUPPORTED_FORMATS:
            if file_format == '.xls':
                msg = ('fastpyxl does not support the old .xls file format, '
                       'please use xlrd to read this file, or convert it to '
                       'the more recent .xlsx file format.')
            elif file_format == '.xlsb':
                msg = ('fastpyxl does not support binary format .xlsb, '
                       'please convert this file to .xlsx format if you want '
                       'to open it with fastpyxl')
            else:
                msg = ('fastpyxl does not support %s file format, '
