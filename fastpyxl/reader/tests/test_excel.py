...
from fastpyxl.utils.exceptions import InvalidFileException
...
def test_load_workbook_from_fileobj(datadir, load_workbook):
    ...
    https://bitbucket.org/fastpyxl/fastpyxl/issue/433
    """
    datadir.chdir()
...
def test_style_assignment(datadir, load_workbook):
    ...
    # Stylesheet defines seven border records (see xl/styles.xml in the xlsx).
...
def test_x14_only_data_validation_extension_loads_without_warning(load_workbook, recwarn):
    import io
    import zipfile
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/>
    ...
    assert len(recwarn) == 0
    dvs = list(wb.active.data_validations.dataValidation)
    assert len(dvs) == 1
    assert dvs[0].type == "list"
    assert dvs[0].formula1 == "Sheet2!$A$1:$A$5"
    assert str(dvs[0].sqref) == "A1"
