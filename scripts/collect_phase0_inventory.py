#!/usr/bin/env python3
"""Generate Phase 0 inventory artifacts for typed serialisable rewrite."""
...
def _parse_module(path: Path) -> ModuleSummary:
...
def _inventory() -> list[ModuleSummary]:
    summaries: list[ModuleSummary] = []
    for path in PACKAGE_ROOT.rglob("*.py"):
        if "tests" in path.parts:
            continue
        summaries.append(_parse_module(path))
    return summaries


def _write_field_mapping(path: Path) -> None:
    lines = [
...
def _write_hard_cases(path: Path, summaries: list[ModuleSummary]) -> None:
    ...
    lines.append("## Notes")
    lines.append("")
    lines.append("- Work package keys follow `artifacts/plan.md` (A: multi-sequence, B: style side effects, C: dynamic values, D: ordering parity).")
    lines.append("- Work package D classes should be added during migration waves as explicit `__elements__`/`__attrs__` overrides are encountered.")
    lines.append("")
...
def main() -> None:
    ...
    summaries = _inventory()
    _write_field_mapping(ARTIFACTS_DIR / "phase0_field_mapping.md")
    _write_hard_cases(ARTIFACTS_DIR / "phase0_hard_case_modules.md", summaries)
    print("Wrote artifacts/phase0_field_mapping.md")
    print("Wrote artifacts/phase0_hard_case_modules.md")
