...
`fastpyxl` is a pure-Python library (a drop-in replacement for `openpyxl >= 3.0.0` on Python `>= 3.11`) with no runtime services, web server, database, or CLI. "Running" it means importing the API or exercising the test/benchmark scripts.
