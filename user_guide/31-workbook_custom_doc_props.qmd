...
It is possible to add one or more CustomDocumentProperty objects to a
workbook. These require a unique name (string) and can be one of 6
types:
...
These properties are globally for a workbook and accessed from the
[custom_doc_props]{.title-ref} attribute.
...
Currently not all possible property types are supported. If fastpyxl
cannot read a particular type, it will provide a warning and ignore it.
::::
